...
:::{note}
SysIdRoutine will name the entry "sysid-test-state-mechanism", where "mechanism" is the name passed to the `Mechanism` constructor or the subsystem name.
:::

```{image} images/log-loaded.png
:alt: Log Loader and Data Selector panes showing the test state entry and where to
:  move it
```
...
:alt: The Data Selector pane after the Position, Velocity, and Voltage entries have
:  been selected
```
